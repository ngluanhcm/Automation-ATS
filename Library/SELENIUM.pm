...
sub takeScreenshot() {
    ...
    $self->{conn}->print("pscp.exe $args{-path} $args{-username}\@$args{-ip}:$locallogname");#
    unless ($self->{conn}->waitfor(-match => '/assword:/', -timeout => 20)) {
        $logger->error(__PACKAGE__ . ".$sub_name: Didnt get 'password: ' prompt. Failed to transfer img file to ATS logs");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    }
    unless (/No such file or directory|unable to open/, $self->{conn}->cmd($args{-password})) {
...
=head2 SonusQA::SELENIUM::quit()
  ...
  This is used to quit the current session
...
sub quit() {
	...
	my $cmd = "java -jar selenium.jar quit $self->{SESSION_ID} $self->{LOCAL_URL}";
    unless ($self->runSeleniumJar($cmd)){
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to quit the current session");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub. [0]");
        return 0;
    }
    $logger->info(__PACKAGE__ . ".$sub_name: Perform quit session successfully ");
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub. [1]");    
}
...
=head2 SonusQA::SELENIUM::sikuliClick()

  Use Sikuli to click on an input image.
  
=item Agruments
    Mandatory: 
                -imagePath: path to the image
    Optional:
    			-similarity: Similarity percent (0.7 by default)           
    			-offsetX: Target offset x (0 by default)           
    			-offsetY: Target offset y (0 by default)           
=item Returns
                - 1 if passed
                - 0 if failed

=item Example:
                $obj->sikuliClick(-imagePath => "C:\Users\Downloads\Picture.PNG");
=back
...
sub sikuliClick() {
    ...
    my $cmd = "java -jar selenium.jar sikuliClick \"$args{-imagePath}\" \"$args{-similarity}\" \"$args{-offsetX}\" \"$args{-offsetY}\"";
    unless ($self->runSeleniumJar($cmd)){    
        $logger->error(__PACKAGE__ . ".$sub_name: Can not click element '$args{-imagePath}'");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    } 
    
    $logger->info(__PACKAGE__ . ".$sub_name: Click element '$args{-imagePath}' successfully ");
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [1]");
    return 1;
}
...
=head2 SonusQA::SELENIUM::sikuliRightClick()

  Right-click on an input image.
...
=item Example:
                $obj->sikuliRightClick(-imagePath => "C:\Users\Downloads\Picture.PNG");
=back

=cut

sub sikuliRightClick() {

    my ($self , %args ) = @_;
    my $sub_name = "sikuliRightClick";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");
    ...
    my $cmd = "java -jar selenium.jar sikuliRightClick \"$args{-imagePath}\" \"$args{-similarity}\" \"$args{-offsetX}\" \"$args{-offsetY}\"";
    unless ($self->runSeleniumJar($cmd)){    
        $logger->error(__PACKAGE__ . ".$sub_name: Can not right-click element '$args{-imagePath}'");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    } 
    
    $logger->info(__PACKAGE__ . ".$sub_name: Right-click element '$args{-imagePath}' successfully ");
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [1]");
    return 1;
}
=head2 SonusQA::SELENIUM::sikuliDoubleClick()

  Double-click on an input image.
...
=item Example:
                $obj->sikuliDoubleClick(-imagePath => "C:\Users\Downloads\Picture.PNG");
=back

=cut

sub sikuliDoubleClick() {

    my ($self , %args ) = @_;
    my $sub_name = "sikuliDoubleClick";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");
    ...
    my $cmd = "java -jar selenium.jar sikuliDoubleClick \"$args{-imagePath}\" \"$args{-similarity}\" \"$args{-offsetX}\" \"$args{-offsetY}\"";
    unless ($self->runSeleniumJar($cmd)){    
        $logger->error(__PACKAGE__ . ".$sub_name: Can not right-click element '$args{-imagePath}'");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
...
=head2 SonusQA::SELENIUM::sikuliType()

  Input text on an image.
...
=item Example:
                $obj->sikuliType(-imagePath => "C:\Users\Downloads\Picture.PNG", -content => "Download");
=back
...
sub sikuliType() {
    ...
    my $cmd = "java -jar selenium.jar sikuliType \"$args{-imagePath}\" \"$args{-similarity}\" \"$args{-offsetX}\" \"$args{-offsetY}\" \"$args{-content}\"";
    unless ($self->runSeleniumJar($cmd)){    
        $logger->error(__PACKAGE__ . ".$sub_name: Can not input '$args{-content}' to '$args{-imagePath}'");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    } 
    
    $logger->info(__PACKAGE__ . ".$sub_name: Input '$args{-content}' to '$args{-imagePath}' successfully ");
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [1]");
    return 1;
}


=head2 SonusQA::SELENIUM::dragDrop()

  Use Sikuli drag an object and drop it at a specific location.
  
=item Agruments
    Mandatory: 
                -imagePath_Drag: path to the image to drag
                -imagePath_Drop: path to the image to drop
    Optional:
    			-similarity_Drag: Similarity percent (0.7 by default)           
    			-offsetX_Drag: Target offset x (0 by default)           
    			-offsetY_Drag: Target offset y (0 by default) 
    			-similarity_Drop:
    			-offsetX_Drop:         
    			-offsetY_Drop:  			
...
=item Example:
                $obj->dragDrop(-imagePath_Drag => "C:\Users\Downloads\Picture.PNG", -imagePath_Drop => "C:\Users\Downloads\Picture1.PNG");
=back
...
sub dragDrop() {
    ...
    my ($self , %args ) = @_;
    my $sub_name = "dragDrop";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");
    
    $args{-similarity_Drag} ||= "0.7";
    $args{-offsetX_Drag} ||= "0";
    $args{-offsetY_Drag} ||= "0";
    $args{-similarity_Drop} ||= "0.7";
    $args{-offsetX_Drop} ||= "0";
    $args{-offsetY_Drop} ||= "0";    
	
    my $flag = 1;
    foreach ('-imagePath_Drag', '-imagePath_Drop') {
        unless ($args{$_}) {
            $logger->error(__PACKAGE__ . ".$sub_name: Mandatory parameter '$_' not present");
            $flag = 0;
            last;
    ...
    my $cmd = "java -jar selenium.jar dragDrop \"$args{-imagePath_Drag}\" \"$args{-similarity_Drag}\" \"$args{-offsetX_Drag}\" \"$args{-offsetY_Drag}\" \"$args{-imagePath_Drop}\" \"$args{-similarity_Drop}\" \"$args{-offsetX_Drop}\" \"$args{-offsetY_Drop}\"";
    unless ($self->runSeleniumJar($cmd)){    
        $logger->error(__PACKAGE__ . ".$sub_name: Can not drag '$args{-imagePath_Drag}' and drop it at '$args{-imagePath_Drop}'");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    } 
    
    $logger->info(__PACKAGE__ . ".$sub_name: Drag '$args{-imagePath_Drag}' and drop it at '$args{-imagePath_Drop}' successfully ");
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [1]");
    return 1;
}

=head2 SonusQA::SELENIUM::wait()

  Use Sikuli to wait till an object appears.
...
=item Agruments
    ...
    Optional:
    			-similarity: Similarity percent (0.7 by default)           
    			-duration: wait for an amount of time (3s by default)           
=item Returns
                - 1 if passed
                - 0 if failed

=item Example:
                $obj->wait(-imagePath => "C:\Users\Downloads\Picture.PNG", -duration => 5);
=back
...
sub wait() {
    ...
    my $cmd = "java -jar selenium.jar wait \"$args{-imagePath}\" \"$args{-similarity}\" \"$args{-duration}\"";
    unless ($self->runSeleniumJar($cmd)){    
        $logger->error(__PACKAGE__ . ".$sub_name: '$args{-imagePath}' does not appear after $args{-duration}s");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    } 
    
    $logger->info(__PACKAGE__ . ".$sub_name: Wait for '$args{-imagePath}' to appear successfully ");
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [1]");
    return 1;
}


=head2 SonusQA::SELENIUM::sikuliScreenshot()

  Use Sikuli to click on an input image.
  
=item Agruments
    Mandatory: 
                -imagePath: Distination path to the image
=item Returns
                - 1 if passed
                - 0 if failed

=item Example:
                $obj->sikuliScreenshot(-imagePath => "C:\Users\Downloads\Picture.PNG");
=back

=cut

sub sikuliScreenshot() {

    my ($self , %args ) = @_;
    my $sub_name = "sikuliScreenshot";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");
    
    my $flag = 1;
    foreach ('-imagePath', '-ip', '-username', '-password') {
        unless (defined $args{$_}) {
            $logger->error(__PACKAGE__ . ".$sub_name: Mandatory parameter '$_' not present");
            $flag = 0;
            last;
    ...
    my $cmd = "java -jar selenium.jar sikuliScreenshot \"$args{-imagePath}\"";
    unless ($self->runSeleniumJar($cmd)){    
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to Take screenshot ");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
	...
	my $cmd = "echo y | pscp -pw $args{-password} $args{-imagePath} $args{-username}\@$args{-ip}:$locallogname";
    unless (/No such file or directory|unable to open/, $self->{conn}->cmd($cmd)) {
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to transfer img file to ATS logs");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    }
    unless ($self->{conn}->cmd("del $args{-imagePath}")) {
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to delete img file in window server");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    }
...
sub initializeAppium() {

    my ($self, %args) = @_;
    my $sub_name = "initializeAppium";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");
    ...
    my $flag = 1;
    foreach ('-appPackage', '-appActivity') {
        unless (defined $args{$_}) {
            $logger->error(__PACKAGE__ . ".$sub_name: Mandatory parameter '$_' not present");
            $flag = 0;
            last;
    ...
    # }
    my @output;
    unless (@output = $self->{conn}->cmd("java -Daction=openapp -DappPackage=$args{-appPackage} -DappActivity=$args{-appActivity} -jar Demo.jar")) {
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to open App");
        $logger->debug(__PACKAGE__ . ".$sub_name: errmsg: " . $self->{conn}->errmsg);
        $logger->debug(__PACKAGE__ . ".$sub_name: Session Dump Log is : $self->{sessionLog1}");
        $logger->debug(__PACKAGE__ . ".$sub_name: Session Input Log is: $self->{sessionLog2}");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub. [0]");
...
sub runAppiumJar() {
    ...
    unless (grep /Response Status Code: 200/, @output){
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub. [0]");
        return 0;
    }  
...
sub appiumClickElement() {

    my ($self, %args ) = @_;
    my $sub_name = "appiumClickElement";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");
    ...
    my $cmd = "java -DsessionID=$self->{SESSION_ID} -Daction=click -Dxpath=\"$args{-xpath}\" -jar Demo.jar";
    unless ($self->runAppiumJar($cmd)) {    
        $logger->error(__PACKAGE__ . ".$sub_name: Can not click element '$args{-xpath}'");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    } 
...
sub appiumInputElement() {

    my ($self, %args ) = @_;
    my $sub_name = "appiumInputElement";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");
    
    my $flag = 1;
    foreach ('-xpath', '-inputValue') {
        unless ($args{$_}) {
            $logger->error(__PACKAGE__ . ".$sub_name: Mandatory parameter '$_' not present");
            $flag = 0;
            last;
    ...
    my $cmd = "java -DsessionID=\"$self->{SESSION_ID}\" -Daction=\"sendkeys\" -Dxpath=\"$args{-xpath}\" -Dvalue=\"$args{-inputValue}\" -jar Demo.jar";
    unless ($self->runAppiumJar($cmd)) {    
        $logger->error(__PACKAGE__ . ".$sub_name: Can not input value '$args{-inputValue}'");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    } 
    
    $logger->info(__PACKAGE__ . ".$sub_name: Input value '$args{-inputValue}' successfully ");
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [1]");
    return 1;
}
...
sub appiumTap() {
    ...
    my $cmd = "java -DsessionID=$self->{SESSION_ID} -Daction=tap -DpointX=$args{-pointX} -DpointY=$args{-pointY} -jar Demo.jar";
    unless ($self->runAppiumJar($cmd)) {    
        $logger->error(__PACKAGE__ . ".$sub_name: Can not perform tap action");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    } 
    
    $logger->info(__PACKAGE__ . ".$sub_name: Perform tap action at pointX=$args{-pointX} and pointY=$args{-pointY} successfully ");
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [1]");
    return 1;
}
...
sub appiumSwipe() {
    ...
    my $cmd = "java -DsessionID=$self->{SESSION_ID} -Daction=swipe -DstartX=$args{-startX} -DstartY=$args{-startY} -DendX=$args{-endX} -DendY=$args{-endY} -jar Demo.jar";
    unless ($self->runAppiumJar($cmd)) {    
        $logger->error(__PACKAGE__ . ".$sub_name: Can not perform swipe action");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    } 
    
    $logger->info(__PACKAGE__ . ".$sub_name: Perform swipe action successfully ");
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [1]");
    return 1;
}
...
sub appiumLongTap() {
    ...
    my $cmd = "java -DsessionID=$self->{SESSION_ID} -Daction=longtap -Dduration=$args{-duration} -Dxpath=\"$args{-xpath}\" -jar Demo.jar";
    unless ($self->runAppiumJar($cmd)) {    
        $logger->error(__PACKAGE__ . ".$sub_name: Can not perform longtap action");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    } 
    
    $logger->info(__PACKAGE__ . ".$sub_name: Perform longtap action successfully ");
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [1]");
    return 1;
}
...
sub appiumQuit() {
    ...
    my ($self , %args ) = @_;
    my $sub_name = "appiumQuit";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");

	my $cmd = "java -Daction=quit -DsessionID=$self->{SESSION_ID} -jar Demo.jar";
    unless ($self->runAppiumJar($cmd)){
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to quit the current session");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub. [0]");
        return 0;
...
sub appiumHideKeyboard() {

    my ($self, %args ) = @_;
    my $sub_name = "appiumHideKeyboard";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");
    
    my $cmd = "java -Daction=hidekeyboard -DsessionID=$self->{SESSION_ID} -jar Demo.jar";
    unless ($self->runAppiumJar($cmd)) {    
        $logger->error(__PACKAGE__ . ".$sub_name: Can not perform hideKeyboard action");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    } 
    
    $logger->info(__PACKAGE__ . ".$sub_name: Perform hideKeyboard action successfully ");
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [1]");
    return 1;
}

sub appiumIsItemDisplayed() {

    my ($self, %args ) = @_;
    my $sub_name = "appiumIsItemDisplayed";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");
    my $flag = 1;
    foreach ('-xpath') {
    ...
    my @content;
    my $cmd = "java -DsessionID=$self->{SESSION_ID} -Daction=islementdisplayed -Dxpath=\"$args{-xpath}\" -jar Demo.jar";
    
    unless (@content = $self->runAppiumJar($cmd)) {    
        $logger->error(__PACKAGE__ . ".$sub_name: Couldn't found the element");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    }

    $logger->info(__PACKAGE__ . ".$sub_name: Element is displayed");
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [1]");
    return 1;
}
...
sub sendingPOSTRequest() {
    my ($self , %args ) = @_;
    my $sub_name = "sendingPOSTRequest";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");

    my $flag = 1;
    foreach ('-requestURL', '-requestBody') {
        unless (defined $args{$_}) {
            $logger->error(__PACKAGE__ . ".$sub_name: Mandatory parameter '$_' not present");
            $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
            $flag = 0;
    ...
    $userAgent->protocols_allowed(['https', 'http']);
    
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- URL : $args{-requestURL} ");
    
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- Body: $args{-requestBody} ");
    
    my $request = HTTP::Request->new(POST => $args{-requestURL});
    
    $request->header( content_type => 'application/json' );
    $request->content($args{-requestBody});
    
    $logger->debug(__PACKAGE__ . ".$sub_name: --> Send POST request to GUI  ");
    my $response = $userAgent->request($request);
    
    $logger->info(__PACKAGE__ . ".$sub_name: --> Send POST request to GUI completely. ");
    $logger->debug(__PACKAGE__ . ".$sub_name: --> Leaving Sub  ");
    return $response;
}

sub sendingGETRequest() {
    my ($self , %args ) = @_;
    my $sub_name = "sendingGETRequest";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");
    ...
    my $request = HTTP::Request->new(GET => $args{-requestURL});
    
    $request->header( content_type => 'application/json' );
    $request->content($args{-requestBody});
    
    $logger->debug(__PACKAGE__ . ".$sub_name: --> Send GET request to GUI  ");
    my $response = $userAgent->request($request);
    
    $logger->info(__PACKAGE__ . ".$sub_name: --> Send GET request to GUI completely. ");
    $logger->debug(__PACKAGE__ . ".$sub_name: --> Leaving Sub  ");
    return $response;
...
sub openApp() {
    ...
    $logger->debug(__PACKAGE__ . ".$sub_name: ==loginnnnn ===  : ".$self->{NODE}->{1}->{IP});
    
    my $flag = 1;
    foreach ('-ip', '-port', '-appPackage', '-appActivity', '-uuid') {
        unless (defined $args{$_}) {
            $logger->error(__PACKAGE__ . ".$sub_name: Mandatory parameter '$_' not present");
            $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
            $flag = 0;
    ...
    my $requestURL = "http://$args{-ip}:$args{-port}/wd/hub/session/";
    # my $requestBody = "{\"desiredCapabilities\": {\"appPackage\": \"$args{-appPackage}\",\"appActivity\": \"$args{-appActivity}\",\"clearSystemFiles\": true,\"noReset\": true,\"platformName\": \"ANDROID\",\"deviceName\": \"Galaxy\",\"platform\": \"ANDROID\", \"uuid\": \"$args{-uuid}\"}}";
    my $requestBody = "{\"desiredCapabilities\": {\"appPackage\": \"$args{-appPackage}\",\"appActivity\": \"$args{-appActivity}\",\"clearSystemFiles\": true,\"noReset\": true,\"platformName\": \"ANDROID\",\"deviceName\": \"$args{-uuid}\",\"platform\": \"ANDROID\",\"uuid\": \"$args{-uuid}\", \"platformVersion\": \"$args{-platformVersion}\"}}";
    
    $logger->debug(__PACKAGE__ . ".$sub_name: ==request body ===  : ".$requestBody);
    
    my $response = $self->sendingPOSTRequest(-requestURL => $requestURL, -requestBody => $requestBody);
       
    unless ($response->code == 200) {
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to Open App using  API ");
        $logger->debug(__PACKAGE__ . ".$sub_name: Response code : ".$response->code);
        $logger->debug(__PACKAGE__ . ".$sub_name: Response body : ".$response->content);
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub. [0]");
        return 0;
    }
    
    $logger->debug(__PACKAGE__ . ".$sub_name: Response body : ".$response->content);
    ($self->{SESSION_ID}) = $response->content =~ /sessionId.*:"(.*)"/;
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- ===== ".$self->{SESSION_ID});
    unless (defined ($self->{SESSION_ID})) {
        $logger->error(__PACKAGE__ . ".$sub_name: '$self->{SESSION_ID}' not defined");
         $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    }        
...
sub findElementBySessionID() {

    my ($self , %args ) = @_;
    my $sub_name = "findElementBySessionID";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");

    my $flag = 1;
    foreach ('-ip', '-port', '-locatedByUsing', '-inputSelectorString') {
        unless (defined $args{$_}) {
            $logger->error(__PACKAGE__ . ".$sub_name: Mandatory parameter '$_' not present");
            $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
            $flag = 0;
    ...
    my $requestURL = "http://$args{-ip}:$args{-port}/wd/hub/session/$self->{SESSION_ID}/element";
    my $requestBody = "{\"using\":\"$args{-locatedByUsing}\",\"value\": \"$args{-inputSelectorString}\"}";
    ...
    unless ($response->code == 200) {
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to Find Element by using $args{-locatedByUsing}: $args{-inputSelectorString}");
        $logger->debug(__PACKAGE__ . ".$sub_name: Response code : ".$response->code);
        $logger->debug(__PACKAGE__ . ".$sub_name: Response body : ".$response->content);
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub. [0]");
        return 0;
    ...
    $logger->debug(__PACKAGE__ . ".$sub_name: Response body : ".$response->content);
    ($self->{ELEMENT_ID}) = $response->content =~ /"ELEMENT":\s?"([\w-]+)"/;
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- ===== ".$self->{ELEMENT_ID});
    unless (defined ($self->{ELEMENT_ID})) {
        $logger->error(__PACKAGE__ . ".$sub_name: '$self->{ELEMENT_ID}' not defined");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    }
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [1]");
...
sub findElementBySessionID() {
    ...
    my $flag = 1;
    foreach ('-ip', '-port', '-locatorStrategy', '-inputString') {
        unless (defined $args{$_}) {
            $logger->error(__PACKAGE__ . ".$sub_name: Mandatory parameter '$_' not present");
            $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
            $flag = 0;
    ...
    my $requestURL = "http://$args{-ip}:$args{-port}/wd/hub/session/$self->{SESSION_ID}/element";
    my $requestBody = "{\"using\":\"xpath\",\"value\": \"$args{-xpath}\"}";
    ...
    unless ($response->code == 200) {
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to Find Element of xpath:  $args{xpath}");
        $logger->debug(__PACKAGE__ . ".$sub_name: Response code : ".$response->code);
        $logger->debug(__PACKAGE__ . ".$sub_name: Response body : ".$response->content);
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub. [0]");
        return 0;
...
sub clickElementApp() {
    ...
    unless ($self->findElementBySessionID(-ip=>$args{-ip}, -port=>$args{-port}, -locatedByUsing=>$args{-locatedByUsing}, -inputSelectorString=>$args{-inputSelectorString})) {
        $logger->error(__PACKAGE__ . "Can't excecute findElementBySessionID");
        return 0;
    }

    my $requestURL = "http://$args{-ip}:$args{-port}/wd/hub/session/$self->{SESSION_ID}/element/$self->{ELEMENT_ID}/click";
    my $requestBody = "";
    
    my $response = $self->sendingPOSTRequest(-requestURL => $requestURL, -requestBody => $requestBody);
       
    unless ($response->code == 200) {
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to Click element by xpath: $args{-xpath} ");
        $logger->debug(__PACKAGE__ . ".$sub_name: Response code : ".$response->code);
        $logger->debug(__PACKAGE__ . ".$sub_name: Response body : ".$response->content);
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub. [0]");
        return 0;
...
sub inputElement() {
    my ($self , %args ) = @_;
    my $sub_name = "inputElement";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");

    my $flag = 1;
    foreach ('-ip', '-port', '-locatedByUsing', '-inputSelectorString', '-inputValue') {
        unless (defined $args{$_}) {
            $logger->error(__PACKAGE__ . ".$sub_name: Mandatory parameter '$_' not present");
            $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
            $flag = 0;
    ...
    my $requestURL = "http://$args{-ip}:$args{-port}/wd/hub/session/$self->{SESSION_ID}/element/$self->{ELEMENT_ID}/value";
    my $requestBody = "{\"id\":\"$self->{ELEMENT_ID}\",\"text\":\"$args{-inputValue}\"}";
...
sub hideKeyboardA() {
    my ($self , %args ) = @_;
    my $sub_name = "hideKeyboardA";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");

    my $requestURL = "http://$args{-ip}:$args{-port}/wd/hub/session/$self->{SESSION_ID}/appium/device/hide_keyboard";
    my $requestBody = "";
    
    my $response = $self->sendingPOSTRequest(-requestURL => $requestURL, -requestBody => $requestBody);
       
    unless ($response->code == 200) {
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to hide keyboard ");
        $logger->debug(__PACKAGE__ . ".$sub_name: Response code : ".$response->code);
        $logger->debug(__PACKAGE__ . ".$sub_name: Response body : ".$response->content);
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub. [0]");
        return 0;
...
sub getCoordinate() {
    my ($self , %args ) = @_;
    my $sub_name = "getCoordinate";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");

    my $requestURL = "http://$args{-ip}:$args{-port}/wd/hub/session/$self->{SESSION_ID}/window/current/size";
    my $requestBody = "";
    
    my $response = $self->sendingGETRequest(-requestURL => $requestURL, -requestBody => $requestBody);
       
    unless ($response->code == 200) {
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to get window size");
        $logger->debug(__PACKAGE__ . ".$sub_name: Response code : ".$response->code);
        $logger->debug(__PACKAGE__ . ".$sub_name: Response body : ".$response->content);
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub. [0]");
        return 0;
    ...
    my ($height)= $response->content =~ /"height":\s?(\d+)/;
    my ($width) = $response->content =~ /"width":\s?(\d+)/;

    my $startPoint = $width * $args{-pointX};
    my $endPoint = $height * $args{-pointY};
...
sub tap() {
    ...
    my ($pointX, $pointY) = $self->getCoordinate(-ip=>$args{-ip}, -port=>$args{-port}, -pointX=>$args{-pointX}, -pointY=>$args{-pointY});

    my $requestURL = "http://$args{-ip}:$args{-port}/wd/hub/session/$self->{SESSION_ID}/touch/perform";
    my $requestBody = "{\"actions\":[{\"action\":\"tap\",\"options\":{\"x\":\"$pointX\",\"y\":\"$pointY\"}}]}";
...
sub longTapElement() {
    ...
    my $requestURL = "http://$args{-ip}:$args{-port}/wd/hub/session/$self->{SESSION_ID}/touch/perform";
    my $requestBody = "{\"actions\":[{\"action\":\"longPress\",\"options\":{\"element\":\"$self->{ELEMENT_ID}\"}},{\"action\":\"wait\",\"options\":{\"ms\":\"$args{-duration}\"}},{\"action\":\"release\",\"options\":{}}]}";
    
    my $response = $self->sendingPOSTRequest(-requestURL => $requestURL, -requestBody => $requestBody);
       
    unless ($response->code == 200) {
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to long tap on element $self->{ELEMENT_ID}");
        $logger->debug(__PACKAGE__ . ".$sub_name: Response code : ".$response->code);
        $logger->debug(__PACKAGE__ . ".$sub_name: Response body : ".$response->content);
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub. [0]");
        return 0;
...
sub implicitWait() {
    my ($self , %args ) = @_;
    my $sub_name = "implicitWait";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");

    $args{-duration} ||=3000;

    my $requestURL = "http://$args{-ip}:$args{-port}/wd/hub/session/$self->{SESSION_ID}/timeouts/implicit_wait";
    my $requestBody = "{\"ms\":$args{-timeout}}";
    
    my $response = $self->sendingPOSTRequest(-requestURL => $requestURL, -requestBody => $requestBody);
       
    unless ($response->code == 200) {
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to set implicitWait");
        $logger->debug(__PACKAGE__ . ".$sub_name: Response code : ".$response->code);
        $logger->debug(__PACKAGE__ . ".$sub_name: Response body : ".$response->content);
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub. [0]");
        return 0;
...
sub isElementDisplayed() {
    my ($self , %args ) = @_;
    my $sub_name = "isElementDisplayed";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");
    ...
    my $requestURL = "http://$args{-ip}:$args{-port}/wd/hub/session/$self->{SESSION_ID}/element/$self->{ELEMNENT_ID}/displayed";
    my $requestBody = "";
    
    my $response = $self->sendingGETRequest(-requestURL => $requestURL, -requestBody => $requestBody);
       
    unless ($response->code == 200) {
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to check if Element is Display");
        $logger->debug(__PACKAGE__ . ".$sub_name: Response code : ".$response->code);
        $logger->debug(__PACKAGE__ . ".$sub_name: Response body : ".$response->content);
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub. [0]");
        return 0;
...
sub clearElementValue() {
    my ($self , %args ) = @_;
    my $sub_name = "clearElementValue";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");
    ...
    my $requestURL = "http://$args{-ip}:$args{-port}/wd/hub/session/$self->{SESSION_ID}/element/$self->{ELEMENT_ID}/clear";
    my $requestBody = "";
...
sub getAttributeA() {
    my ($self , %args ) = @_;
    my $sub_name = "getAttributeA";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");

    my $flag = 1;
    foreach ('-ip', '-port', '-locatedByUsing', '-inputSelectorString', '-attribute') {
        unless (defined $args{$_}) {
            $logger->error(__PACKAGE__ . ".$sub_name: Mandatory parameter '$_' not present");
            $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
            $flag = 0;
    ...
    my $requestURL = "http://$args{-ip}:$args{-port}/wd/hub/session/$self->{SESSION_ID}/element/$self->{ELEMENT_ID}/attribute/$args{-attribute}";
    my $requestBody = "";
    
    my $response = $self->sendingGETRequest(-requestURL => $requestURL, -requestBody => $requestBody);
       
    unless ($response->code == 200) {
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to get attribute");
        $logger->debug(__PACKAGE__ . ".$sub_name: Response code : ".$response->code);
        $logger->debug(__PACKAGE__ . ".$sub_name: Response body : ".$response->content);
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub. [0]");
        return 0;
    ...
    my ($attribute) = $response->content =~ /"value":\s?"([\w-]+)"/;
    $logger->debug(__PACKAGE__ . ".$sub_name: <-- ===== ".$attribute);
    
    unless (defined ($attribute)) {
        $logger->error(__PACKAGE__ . ".$sub_name: Could not get atribute");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
    }
...
sub swipe() {
    ...
    my ($startX, $startY) = $self->getCoordinate(-ip=>$args{-ip}, -port=>$args{-port}, -pointX=>$args{-startX}, -pointY=>$args{-startY});
    my ($endX, $endY) = $self->getCoordinate(-ip=>$args{-ip}, -port=>$args{-port}, -pointX=>$args{-endX}, -pointY=>$args{-endY});

    my $requestURL = "http://$args{-ip}:$args{-port}/wd/hub/session/$self->{SESSION_ID}/touch/perform";
    my $requestBody = "{\"actions\":[{\"action\":\"press\",\"options\":{\"x\":\"$startX\",\"y\":\"$startY\"}},{\"action\":\"wait\",\"options\":{}},{\"action\":\"moveTo\",\"options\":{\"x\":\"$endX\",\"y\":\"$endY\"}},{\"action\":\"release\",\"options\":{}}]}";
...
sub closeApp() {
    my ($self , %args ) = @_;
    my $sub_name = "closeApp";
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->info(__PACKAGE__ . ".$sub_name: --> Entered Sub");

    my $requestURL = "http://$args{-ip}:$args{-port}/wd/hub/session/$self->{SESSION_ID}/appium/app/close";
    my $requestBody = "";
    
    my $response = $self->sendingPOSTRequest(-requestURL => $requestURL, -requestBody => $requestBody);
       
    unless ($response->code == 200) {
        $logger->error(__PACKAGE__ . ".$sub_name: Failed to close app");
        $logger->debug(__PACKAGE__ . ".$sub_name: Response code : ".$response->code);
        $logger->debug(__PACKAGE__ . ".$sub_name: Response body : ".$response->content);
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub. [0]");
        return 0;
