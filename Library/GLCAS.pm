...
=item Example:
    ...
    my ($result_code, $status) = $obj->detectNoTestToneCAS(%args);
...
sub detectNoTestToneCAS {
    ...
    unless ($result_code == 1 || grep /Null/, @output){
        $logger->error(__PACKAGE__ . ".$sub_name: Fail reason is $status_cmd{$result_code} ");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving sub [0]");
        return 0;
    }
...
=head2 B<makeCall()>
    ...
    Input:
        ...
        - Detection type:
            'DELAY 2' : Delay 2 ms before checking ringback/ringing tone.
        ...
        - Ring off
            ...
            Default is 0.
        - OnOff hook
            ['offB'] : it will off-hook B (default).
...
sub detectStutterDialToneCAS {
    ...
    my @cmd = (
                ...
                "maps cmd 1 UserEvent $args{-line_port} {\"Detect Stutter Dial Tone\"} # TIMEOUT = $args{-cas_timeout}",
