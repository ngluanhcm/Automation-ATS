...
sub DESTROY {
    ...
    if ($vmCtrlAlias and !$self->{DO_NOT_DELETE} and $main::BISTQ_LAST_SUITE ne 'N') {
        $logger->debug(__PACKAGE__ . ".DESTROY Deleting the Cloud Instance");
        $vm_ctrl_obj{$vmCtrlAlias}->deleteInstance($self->{'CE_NAME'}) if($main::TESTBED{CREATED_BY_ATS}->{$self->{'CE_NAME'}} || $main::BISTQ_LAST_SUITE eq 'Y');
        $vm_ctrl_obj{$vmCtrlMaster}->deleteInstance($self->{TMS_ALIAS_DATA}->{MASTER}->{1}->{NAME}) if($self->{TMS_ALIAS_DATA}->{MASTER}->{1}->{NAME} and $self->{TMS_ALIAS_DATA}->{__OBJTYPE}  eq 'PSX' and ($main::TESTBED{CREATED_BY_ATS}->{$self->{TMS_ALIAS_DATA}->{MASTER}->{1}->{NAME}} or $main::BISTQ_LAST_SUITE eq 'Y'));
        $logger->debug(__PACKAGE__ . ".DESTROY completed Deleting the Cloud Instance");
    }
    if(exists $self->{TMS_ALIAS_DATA}->{__OBJTYPE} && $self->{TMS_ALIAS_DATA}->{__OBJTYPE} eq 'EMS' && $main::TESTBED{CREATED_BY_ATS}->{$self->{PSX_ALIAS}} &&  $main::TESTBED{'FC_CLEANUP'}){ 
        #CAll unregisterPSXNode only if object type is EMS and its a cloud PSX.	
	my @nodes = ($self->{PSX_ALIAS});
	push(@nodes,$self->{OBJ_TARGET}) if($self->{PSX_ALIAS} ne $self->{OBJ_TARGET} && $main::TESTBED{CREATED_BY_ATS}->{$self->{OBJ_TARGET}});
...
DESCRIPTION:
    ...
    This subroutine is used to execute REST API methods GET/PUT/POST/DELETE. It returns a string with error code and error message . Eg -> 503: Service Unavailable
...
Mandatory :
   ...
   -method      => Type of REST client method  GET/PUT/POST/DELETE
...
sub restAPI {
	...
	$logger->info(__PACKAGE__ . ".$sub Entered sub");
        ...
        unless($args{-method} =~ /GET|PUT|POST|DELETE/){
        ...
        unless($args{-username}){
                $logger->error("$sub: Invalid username ");
                $logger->info("$sub: <-- Leaving Sub[0]");
                return 0;
        }

        unless($args{-password}){
                $logger->error("$sub: Invalid password ");
                $logger->info("$sub: <-- Leaving Sub[0]");
                return 0;
        }

        my $headers = {Accept => "application/$args{-contenttype}", 'Content-Type' => "application/$args{-contenttype}", Authorization => "Basic " . encode_base64($args{-username} . ":" . $args{-password})};
...
sub enterRootSessionViaSU
{
        ...
        if($self->{'TMS_ALIAS_DATA'}->{'__OBJTYPE'} =~ /(PSX|EMS)/i and !grep(/^uid=\d+\(admin\)/,@cmdresults)){ #TOOLS-17812 TOOLS-17912 
            ...
            unless ( grep( /admin/,@{$self->{CMDRESULTS}} ) )
            {
	...
	$logger->debug( __PACKAGE__ . ".$sub: Issuing the Cli: $cmd2" );
        ...
        unless($self->{conn}->cmd(String => 'bash')){
            $logger->error(__PACKAGE__ . ".$sub: Could not execute 'bash'");
            $logger->debug(__PACKAGE__ . ".$sub: <-- Leaving Sub [0]");
            return 0;
        }

        $self->{conn}->prompt('/AUTOMATION# $/');
        $logger->debug( __PACKAGE__ . ".$sub: changing the prompt to /AUTOMATION# \$/");
        $self->execCmd('export PS1="AUTOMATION# "');
        $self->{conn}->waitfor(Match => '/AUTOMATION# $/', Timeout => 2); #TOOLS-12706 sometimes, prompt is matched with input cmd so added extra waitfor to handle it.
...
sub verifyDecodeMessage{
     ...
     if ($args{-RawData} ) {
         ...
         foreach (@{$args{-String}}) {
             ...
             unless (grep (/$string_addr[0]\s+$string_addr[1]/i, @output)) {
                 $logger->error(__PACKAGE__ . ".$sub_name: search pattern  $_ not present in decoded result");
                 $main::failure_msg .= "UNKNOWN:SBX5000HELPER-Pattern Mismatch; ";
                 return 0;
             }
         }
         $logger->debug(__PACKAGE__ . ".$sub_name: search pattern found in decoded result");
         return 1;
     } else {
     ...
     foreach my $temp_type (keys %temp_string) {
         foreach my $sent_received (keys %{$temp_string{$temp_type}}) {
             ...
             if (ref ($temp) eq 'ARRAY') {
                 ...
                 foreach (@{$temp}) {
                    my @string_addr = split('-', $_);
                    unless (grep (/($string_addr[0]\s+$string_addr[1]|$string_addr[1]\s*\|\s*$string_addr[0])/i, @{$pattern{$temp_type}{$sent_received}{$occurence}})) {
                       $logger->error(__PACKAGE__ . ".$sub_name: search pattern $_ not present in decoded result");
			...
			$main::failure_msg .= "UNKNOWN:SBX5000HELPER-Pattern Mismatch; ";
                 ...
                 foreach my $parmeter_code (keys %{$temp}) {
                     ...
                     foreach (keys %found) {
                         if ($found{$_}){
                             $logger->debug(__PACKAGE__ . ".$sub_name: search pattern $_ found for $parmeter_code -> $sent_received -> $temp_type");
                         } else {
                             $logger->error(__PACKAGE__ . ".$sub_name: search pattern $_ not found for $parmeter_code -> $sent_received -> $temp_type");
				$main::failure_msg .= "UNKNOWN:SBX5000HELPER-Pattern Mismatch; ";
                             return 0;
                         }
                     }
...
sub exitUser {
	...
	unless( $self->becomeUser( -userName => $self->{OBJ_USER}, -password => $self->{OBJ_PASSWORD} ) ){
        ...
        unless( $self->{conn}->cmd( String => 'exit', Prompt => $self->{DEFAULTPROMPT}) ){
...
sub verifyTable {
    ...
    my $logger = Log::Log4perl->get_logger(__PACKAGE__ . ".$sub_name");
    $logger->debug(__PACKAGE__ . ".$sub_name: --> Entered Sub");
    $logger->debug(__PACKAGE__ . ".$sub_name: Args : ". Dumper($self,$cmd,$cliHash,$mode));
    unless ( $self ) {
        $logger->error(__PACKAGE__ . ".$sub_name: Mandatory shell session input is empty or blank.");
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [0]");
        return 0;
...
$logger->debug(__PACKAGE__ . ".$sub_name: Changed '$old_ingress' to '$cliHash{'ingressMediaStream1LocalIpSockAddr'}' for 'ingressMediaStream1LocalIpSockAddr' since it is D_SBC" );
        ...
        if($cmd =~ /ipAccessControlList/i){           #Fix for TOOLS-14515 to change key only if its ipACL
        ...
        unless (@grep_result = grep(/\Q$key\E.+\Q$cliHash{$key}\E/i, @output)) {
            ...
            if (@grep_result = grep(/\Q$key\E.+/i, @output)) {
  			...
  			my @range = @{$cliHash{$key}};
                ...
                $main::failure_msg .=  $self ."-Failed to match $key; ";
                $flag = 1;
            }
        } else {
...
if($flag){
        ...
        %cliHash = %verifyHash;
        $count++;
        $logger->debug(__PACKAGE__ . ".$sub_name: Sleeping for 5s. Count ($count/12)");
        sleep(5);
        goto RETRY;
    }else{
        $logger->debug(__PACKAGE__ . ".$sub_name: <-- Leaving Sub [1]");
